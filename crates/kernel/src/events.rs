...
impl Event {
    ...
    /// record *is* the work — losing it loses everything the agent did. It
    /// carries the parent's own label rather than the child's: the diff is
    /// generated by git from the child's edits, not authored content, and the
    /// child's trust already rides its report.
    pub fn agent_patch(s: &Session, name: &str, child: Ulid, patch: Value) -> Self {
        Self::new(
            s,
            EventKind::AgentPatch,
            json!({ "agent": name, "child": child.to_string(), "patch": patch }),
            TrustLabel::System,
        )
    }
    ...
    pub fn agent_applied(s: &Session, child: Ulid) -> Self {
        Self::new(
            s,
