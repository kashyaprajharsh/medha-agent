...
fn lsp_install_hint(id: &str) -> &'static str {
    match id {
        ...
        "jdtls" => "install Eclipse JDT language server (brew install jdtls)",
        "omnisharp" => "install the .NET SDK, then OmniSharp",
        "lua-language-server" => "brew install lua-language-server, or a GitHub release",
        "zls" => "build zls from source, or a GitHub release matching your zig",
        "sourcekit-lsp" => "ships with Swift/Xcode toolchains",
        _ => "install this server through its own toolchain",
    }
}
...
async fn run_lsp_command(args: &[String]) -> Result<()> {
    ...
    match args.first().map(String::as_str).unwrap_or("status") {
        "list" | "status" => {
            ...
            let installable: Vec<&str> = config
                .servers
                .iter()
                .filter(|adapter| {
            ...
            if !installable.is_empty() {
                println!("  · medha lsp install <id>   ({})", installable.join(", "));
            }
        }
        "install" => {
            let id = args
                .get(1)
                .ok_or_else(|| anyhow::anyhow!("usage: medha lsp install <server-id>"))?;
            let adapter = config
                .servers
                .iter()
                .find(|adapter| &adapter.id == id)
                .ok_or_else(|| {
                    anyhow::anyhow!("no language server '{id}' (see `medha lsp list`)")
                })?;
            let program = adapter.command.first().cloned().unwrap_or_default();
        ...
        other => {
            return Err(anyhow::anyhow!(
                "unknown lsp subcommand '{other}' (use list|status|install)"
            ));
        }
    }
    Ok(())
