//! End-to-end rewind through a SQLite event log and workspace sandbox. Snapshot
//! ids must survive the log/fork/rollback pipeline while the original session
//! remains intact.
...
/// Log a write with the snapshot metadata consumed by `rollback_plan`.
async fn log_write(
    log: &store::SqliteLog,
...
async fn rewind_branches_the_session_and_rolls_files_back_to_the_cut() {
    ...
    // Forking must preserve the tamper-evident event chain.
    log.verify().unwrap();
